...
export default function MDMAccounts() {
  const isFocused = useIsFocused();
  const navigation = useNavigation();
  const { state, setActiveTab, setStateObject, accountState, setAccountState } =
    useGlobalContext();
  ...
  const getAccounts = async () => {
    setLoader(true);
    await getCollection('accounts').then(snapshot => {
  ...
  return (
    <View style={{ flex: 1 }}>
      {loader && <Loader />}
      <Text style={styles.title}>MID DAY MEAL ACCOUNT</Text>
      <ScrollView
        style={{
